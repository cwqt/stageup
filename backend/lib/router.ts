...
const executeAuthenticationStrategy = (authStrategy: AuthStrategy, dc: DataClient) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!isAuthorised)
        throw new ErrorHandler(reason == ErrCode.NO_SESSION // https://developer.mozilla.org/en-US/docs/Web/HTTP/Status
          ? HTTP.Unauthorised         // Semantically means unauthenticated contrary to the HTTP code
          : HTTP.Forbidden, reason);  // Means client is known, but doesn't have access to the resource
