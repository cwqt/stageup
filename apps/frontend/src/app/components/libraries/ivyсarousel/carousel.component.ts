...
export class CarouselComponent implements OnDestroy {
  ...
  handleTap = (event: any) => {
    ...
    if (this.images) {
      let imageIndex = this.cells.getImageIndex(i);
      let file = this.cells.getImage(imageIndex);
  ...
  handleTransitionendCellContainer(event: TransitionEvent) {
    // don't handle events from child elements of the cells container
    // otherwise it'll fuck up all the indexes by trying to scroll when it shouldnt
    if (event.target['className'] !== 'carousel-cells') return;
