...
@Service()
export default class PatronageController extends ModuleController {
  constructor(
  ...
  createPatronTier: IControllerEndpoint<IHostPatronTier> = {
    ...
    controller: async req => {
      ...
      const tier = await this.ORM.transaction(async txc => {
        ...
        return await tier.setup(this.stripe, txc);
      });
  ...
  readPatronTiers: IControllerEndpoint<Array<IHostPatronTier | IPatronTier>> = {
    ...
    controller: async req => {
      ...
      const [isMemberOfHost] = await AuthStrat.hasHostPermission(HostPermission.Admin, () => host._id)(req);
  ...
  updatePatronTier: IControllerEndpoint<IHostPatronTier> = {
    ...
    controller: async req => {
      ...
      const oldTier = await getCheck(PatronTier.findOne({ where: { _id: req.params.tid }, relations: { host: true } }));
      ...
      if (oldTier.amount !== update.amount) {
        try {
          const newTier = await transact(async txc => {
            ...
            await newTier.createStripePrice(this.stripe);
  ...
  deletePatronTier: IControllerEndpoint<void> = {
    ...
    controller: async req => {
      ...
      await this.bus.publish('patronage.tier_deleted', { tier_id: tier._id }, req.locale);
    }
  };
  ...
  subscribe: IControllerEndpoint<IPatronSubscription> = {
    ...
    controller: async req => {
      ...
      const copiedCustomer = await this.stripe.customers.create(
        {
      ...
      this.bus.publish('patronage.started', { user_id: user._id, tier_id: tier._id }, req.locale);
